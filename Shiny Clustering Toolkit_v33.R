...
# 2. Loading the required Dataset -----------------------------------------------------------
    ...
    input_first <- read.spss("C:/Users/Florian Gehrig/Box/Dean Foods (2018)/Quantitative/03_Analysis/DATASET/Final excl Bad Respondents/18121040_Frozen excl Bad_clustered.sav", to.data.frame=TRUE)
    ...
    scales<-vector(mode="list", 100)
        ...
        scales[[1]]<-c("Agree completely","Agree slightly","Agree somewhat","Disagree somewhat","Disagree slightly","Disagree completely")
...
# X.Data Subsetting
        ...
        #Query Dataframe
        names(input_first)[grep("S4QA",names(input_first))] #Name Query
        ...
        beautify <- function(input_data, variable, group_var="") {
          ...
          attributes<- attributes(input_first)[[4]]
          attributes<- attributes[names(attributes) %in% names_temp]
          attributes<- sub(".*:(.*) - .*", "\\1", attributes) %>% as.vector()
        ...
        subset_data<-subselect(input_first,variable = "^S2Q1r.*", relabel = T) %>% sapply(as.numeric) %>% as.data.frame()

        r<-cor(subset_data)
        
        fa.results <- fa(r, nfactors=5, rotate="oblimin",
                         scores=TRUE, fm="ml", oblique.scores=FALSE, max.iter=25)
        factors<-fa.sort(fa.results,polar=FALSE)
        print(factors$loadings,cutoff = round(0.35))

        describe(subset_data)
        subset_data<-subset_data[,-c((ncol(subset_data)-1),ncol(subset_data))] %>% scale()
...
# X. Exploratory Data Analysis
  ...
  # NEW       
  response <- function(input_data, glossar_data, variables, group = "Cluster", benchmark = c("percentage points","index","absolute"), top_n = 2, plots_per_page=4, save = F, nrows = 1, ncols = 2, font_family = "Arial Narrow") {
                  ...
                  for (k in 1:length(scales)) {
                    ...
                    if (setequal(temp$Level,scales[[k]]) == TRUE) {
                      ...
                      tempx<-merge(temp,temp2 %>% select(Grouping,Dimension, mean), all.x = TRUE) %>% group_by(Grouping) %>% mutate(MEANS = mean(mean))
                        ...
                        plot1<-ggplot(temp2, aes(x=Dimension,y=round(p,2),group=factor(Level))) + geom_bar(aes(fill=factor(Level)), stat = "identity", position = "stack") + theme(axis.text.x = element_text(angle = 90, hjust = 1))+ 
                        ...
                        if (too_big == TRUE) {
                          ...
                          for (j in 1:n_pages_needed) {
                            ...
                            plot1<-plot1 + coord_flip() + geom_text(data=temp2,aes(x=Dimension,y=1.08,label=paste("Ø",round(mean,2))),size = 2) + facet_wrap_paginate(~Grouping, nrow = nrows, ncol = ncols, page = j)+
                        ...
                        plot_list[[i]]<-c(temps_plots,plot_bundle1,plot_bundle3)
   ...
   ggthemr_reset()
   motivations<-response(input,glossars,variables = c("^Q13","S4Q4") , group = "Cluster", save = T)
    ...
    efa <- function (input_data, cutoff, n_factors, rotate = "oblimin", fm = "minres", ...) {
      ...
      loads_plot <- loads %>% mutate(i = as.numeric(rownames(loads))) %>% gather("Factor", "Loading", -i)  %>% cbind(df_info[,1]) %>% arrange(i)
      factor_plot <- ggplotly(ggplot(loads_plot, aes(x=factor(Item), y=Loading)) + geom_bar(aes(fill=Factor),stat = "identity")+theme_light() + theme(axis.text.x = element_text(angle = 90, size=7),axis.text.y = element_text(size=7)) + coord_flip())
      ...
      final_html<-browsable(tagList(ggplotly(gplot),style_widget(hw=factor_plot, "font-family:monospace"), style_widget(hw=factor_table, "font-family:monospace"),nrows = 2))
      
      save_html(final_html,"d.html")
      return(df_cutted)
      
    }
    
        data_factorized<-efa(input_data = subset_data,cutoff=0.4,n_factors=9)  ## ggf. Shiny App entwickeln?
...
# 4. Cluster Option Evaluation ------------------------------------------------------------------------------------------------------------------------
    ...
    # KMeans, Hierarchical, PAM, Sota, Diana, Clara, Model
        ...
        subset_data<-subselect(input_first,variable = "^S2Q1r.*", relabel = T) %>% sapply(as.numeric) %>% as.data.frame()
        subset_data<-subset_data[,-c((ncol(subset_data)-1),ncol(subset_data))] %>% sapply(as.numeric) %>% scale()
        subset_data<-subset_data[,-c(7,10,14,17,18,19,24,28,29,31,33,35,36,37)] 
    ...
    cluster_eval <- function(input_data, cluster, clust_methods = c("hierarchical", "kmeans",  "pam", "sota", "diana","clara","model"), val_methods = c("internal"), distances = c("euclidean"), linkage = c("average"), stats = TRUE, plot = TRUE) {
      ...
      return(output)
        ...
        valuation3<-cluster_eval(subset_data, clust_methods = c("hierarchical","agnes", "kmeans",  "pam", "sota", "diana","clara"), distances = c("euclidean"),cluster = 8, plot = T)
...
# 5. Cluster Profiling ----------------------------------------------
    ...
    library(amap)
        cluster_output<-Kmeans(x=subset_data, centers = 5, method = "correlation")$cluster %>% sapply(as.factor) %>% as.data.frame()
